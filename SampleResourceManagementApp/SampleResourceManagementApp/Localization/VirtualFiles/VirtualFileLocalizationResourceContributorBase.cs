...
using SampleResourceManagementApp.Utilities.FileInfoExtensions;
using SampleResourceManagementApp.Utilities.StringExtensions;
using SampleResourceManagementApp.Utilities.Utf8Helpers;
    ...
    public abstract class VirtualFileLocalizationResourceContributorBase : ILocalizationResourceContributor
    {
        ...
        private Dictionary<string, ILocalizationDictionary> CreateDictionaries()
        {
            ...
            foreach (var file in _virtualFileProvider.GetDirectoryContents(_virtualPath))
            {
                ...
                {
                    throw new Exception($"{file.GetVirtualOrPhysicalPathOrNull()} dictionary has a culture name '{dictionary.CultureName}' which is already defined!");
                }
